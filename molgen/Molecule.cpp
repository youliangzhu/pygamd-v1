...
void Molecule::readData(const std::string& fname)
	...
	unsigned int Np;
	std::vector< std::string > typeMapping;
	std::vector<vec> pos;
	...
	std::vector<unsigned int> molecule;
	std::vector<double> charge;
	std::vector<double> diameter;
	std::vector<vec> ori;
	std::vector<vec4> quat;
	std::vector<vec> inert;
	std::vector<unsigned int> cris;
	std::vector<unsigned int> init;
	std::vector<str_vec6> asphere;
	std::vector<str_vec6> patch;
	std::vector<str_vec6> patch_num;
	std::vector<Bond> bonds;
	std::vector<Angle> angles;
	std::vector<Dihedral> dihedrals;
	std::vector<Dihedral> vsites;

	if (xp!=fname.npos)
		{
		cout<<"read xml file "<<fname<<endl;
		XMLBuilder build(fname.c_str());
		
		Np = build.getNParticles();
		typeMapping = build.getTypeMap();
		pos = build.getPos();
		...
		type = build.getType();
		body = build.getBody();
		molecule = build.getMolecule();
		charge = build.getCharge();
		diameter = build.getDiameter();
		ori = build.getOrientation();
		quat = build.getQuaternion();
		inert = build.getInert();
		cris = build.getCris();
		init = build.getInit();
		asphere = build.getAsphere();
		patch = build.getPatch();
		patch_num = build.getPatchNum();
		
		bonds = build.getBond();
		angles = build.getAngle();
		dihedrals = build.getDihedral();
		vsites = build.getVsite();
		}
	else if (mp!=fname.npos)
		{
		cout<<"read mst1 file "<<fname<<endl;
		MSTReader build;
		...
		{
		cerr << endl << "***Error! The file type should be xml or mst !"<< endl << endl;
...
void export_Molecule(pybind11::module &m)
	{
	pybind11::class_<Molecule, std::shared_ptr<Molecule> >(m, "Molecule")
