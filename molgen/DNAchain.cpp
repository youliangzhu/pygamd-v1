...
void DNAchain::allocate_DNAdata()
	...
	m_site_data1[sites::Ph] = vec4(2.186,8.918,94.038,94.97);			//the z ,r ,degree, mass	
	m_site_data1[sites::Su] = vec4(1.280,6.981,70.197,83.11);			//the z ,r ,degree, mass
	...
	m_site_data2[sites::Ph] = vec4(-2.186,8.918,265.962,94.97);			//the z ,r ,degree, mass
	m_site_data2[sites::Su] = vec4(-1.280,6.981,289.803,83.11);			//the z ,r ,degree, mass
	...
	m_site_name[sites::Ph] = "Ph";			
	m_site_name[sites::Su] = "Su";		
...
// the base of first strand
		if(m_circle||i!=m_N_bps-1)
			{
			z = m_site_data1[sites::Ph].x + double(i)*m_delt_z;
			r = m_site_data1[sites::Ph].y;
			d = m_site_data1[sites::Ph].z + double(i)*m_delt_degree;
		...
		z = m_site_data1[sites::Su].x + double(i)*m_delt_z;
		r = m_site_data1[sites::Su].y;
		d = m_site_data1[sites::Su].z + double(i)*m_delt_degree;
		d = d - int(d/360.0)*360.0;
		t = int(sites::Su);
...
// the base of second strand	
		if(m_s==ds)
			...
			if(m_circle||i!=0)
				{
				z = m_site_data2[sites::Ph].x + double(i)*m_delt_z;
				r = m_site_data2[sites::Ph].y;
				d = m_site_data2[sites::Ph].z + double(i)*m_delt_degree;
				d = d - int(d/360.0)*360.0;
				t =int(sites::Ph);
			...
			z = m_site_data2[sites::Su].x + double(i)*m_delt_z;
			r = m_site_data2[sites::Su].y;
			d = m_site_data2[sites::Su].z + double(i)*m_delt_degree;
			d = d - int(d/360.0)*360.0;
			t =int(sites::Su);
...
void export_DNAchain(pybind11::module &m)
	{
	pybind11::class_<DNAchain, Molecule, std::shared_ptr<DNAchain> >(m, "DNAchain")
    ...
    pybind11::enum_<DNAchain::Strand>(m, "Strand")
    .value("ss",DNAchain::Strand::ss)
    .value("ds",DNAchain::Strand::ds)	
	.export_values();
	}
