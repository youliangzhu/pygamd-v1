...
if (len(atomproperties[0])==8):
    print("Debug: Processing atomproperties for name-to-type mapping")
    for i in atomproperties:
        nametotype[i[0]] = i[1]
        print(f"Mapped {i[0]} -> {i[1]}")  # 添加调试输出
    print("Debug: nametotype dictionary:", nametotype)
	...
	for filename in ItpFiles:
		...
		for line in itp:
			...
			if read_bond and len(la)==4:
				...
				elif bool(bond_parameter(nametotype.get(atom[int(la[0])-1][1]), nametotype.get(atom[int(la[1])-1][1]),la[2])):
					# 使用 get() 方法并添加调试信息
					atom1 = atom[int(la[0])-1][1]
					atom2 = atom[int(la[1])-1][1]
					if atom1 not in nametotype or atom2 not in nametotype:
						print(f"Warning: Missing atom type mapping for {atom1} or {atom2}")
						print(f"Available mappings: {list(nametotype.keys())}")
					
					para = bond_parameter(nametotype.get(atom1), nametotype.get(atom2), la[2])
					bond.append([nametotype.get(atom1)+"-"+nametotype.get(atom2)]+la[:2]+para)
				else:
					print(la)
					print(atom[int(la[0])-1][1],atom[int(la[1])-1][1], la[2])
					print(nametotype.get(atom[int(la[0])-1][1]), nametotype.get(atom[int(la[1])-1][1]), la[2]) 
...
for i in range(0, len(Molecules)):
	...
	for k in pair:
		if k[2] == "1":
			pair_type.append([f"{int(k[0])-1}-{int(k[1])-1}"])
			pair_new.append(["p1", int(k[0])-1, int(k[1])-1])
