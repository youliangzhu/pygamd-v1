...
class XMLBuilder 
    ...
    public:
        ...
        unsigned int getLastNBonds() const;
		
		virtual bool ifchangedNp() {return m_if_changed_np; }
		
		virtual bool ifchangedNb() {return m_if_changed_nb; }
        ...
        const std::vector< double >& getVirial() const { return m_virial; }
        const std::vector< std::vector<double> >& getVirialMatrix() const { return m_virial_matrix; }
		
        virtual std::string getFilename() { return m_fname; }
        std::string getFirstFilename(){ return m_fname; }
		void readDataFromXML(const string &fname);
    ...
    protected:
        ...
        void parseRotangleNode(const XMLNode& node);
        void parseRotationNode(const XMLNode& node);
        ...
        void parseVirialNode(const XMLNode& node);
        void parseVirialMatrixNode(const XMLNode& node);
        ...
        std::string m_fname;
        unsigned int m_ndimension;              //!< number of spatial dimensions
		
        std::vector< vec > m_pos;             //!< positions of all particles loaded
        std::vector< vec_int > m_image;       //!< images of all particles loaded
        std::vector< vec > m_vel;             //!< velocities of all particles loaded
        std::vector< double > m_mass;         //!< masses of all particles loaded
        std::vector< double > m_diameter;     //!< diameters of all particles loaded
        std::vector< unsigned int > m_type;   //!< type values for all particles loaded
        std::vector< unsigned int > m_body;   //!< body values for all particles loaded		
        std::vector< double > m_charge;       //!< charge of the particles loaded
        ...
        std::vector< std::vector<double> > m_virial_matrix;
        ...
        std::vector<std::string> m_type_map;          //!< The created map between particle types and ids
        std::vector<std::string> m_bond_type_map;     //!< The created map between bond types and ids
        std::vector<std::string> m_angle_type_map;    //!< The created map between angle types and ids
        std::vector<std::string> m_dihedral_type_map; //!< The created map between dihedral types and ids
        std::vector<std::string> m_vsite_type_map; //!< The created map between vsite types and ids
		std::string m_node_name;
		std::string m_object_name;
		bool m_if_changed_np;
		bool m_if_changed_nb;
		unsigned int m_last_np;
		unsigned int m_last_nb;
